class Filter {
    ...
    static percentile(vars) {
        const minFloat =
            vars.minfloat > vars.minwearfloat
                ...
                : vars.minwearfloat;
        const maxFloat =
            vars.maxfloat < vars.maxwearfloat
                ? vars.maxfloat
                : vars.maxwearfloat;
        const itemPercentile =
            100 - (100 * (vars.float - minFloat)) / (maxFloat - minFloat);
    ...
    compileExpression() {
        this.filtrexFunc = compileExpression(
            this.expression,
            this.validExpressionVars
        );
    }
    ...
    addToUI() {
        ...
        let globalToggleBtn = createButton(
            'Global',
            this.isGlobal ? 'green' : 'grey'
        );
        globalToggleBtn.addEventListener('click', e => {
            globalToggleBtn.classList.remove(
                `btn_${this.isGlobal ? 'green' : 'grey'}_white_innerfade`
            );
            this.isGlobal = !this.isGlobal;
            globalToggleBtn.classList.add(
                `btn_${this.isGlobal ? 'green' : 'grey'}_white_innerfade`
            );
        ...
        tooltipText.innerText =
            'Global filters apply to every item on the market';
...
class Filters {
    ...
    getMatchColour(vars) {
        ...
        if (colours.length > 0) {
            ...
            for (let index in avg_colours) {
                avg_colours[index] = parseInt(
                    avg_colours[index] / colours.length
                );
            }
    ...
    addFilter(expression, colour) {
        if (arguments.length === 0) {
            expression = document.querySelector('#float_expression_filter')
                .value;
    ...
    async addFilterUI(parent) {
        ...
        const localFilters = await this.getSavedFilters();
    ...
    getSaveKey() {
        let itemName = document.querySelector(
            '.market_listing_nav a:nth-child(2)'
        );
