...
const addAutoFill = function (trades, isSeller) {
...
const addFloatMarketFill = async function () {
    ...
    if (tradeStatus.me.assets.length > 0) {
        ...
        for (const item of tradeStatus.me.assets) {
            const hasTrade = partnerTrades.find(e => e.contract.offering.asset_id === item.assetid);
    ...
    addAutoFill(partnerTrades, true);

    // Auto-fill for buyers
    addAutoFill(data.trades_to_receive.filter(e => e.seller_id === partnerId), false);
};
