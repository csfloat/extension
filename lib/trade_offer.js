...
const addFloatMarketFill = async function () {
    ...
    const partnerTrades = data.trades_to_send.filter(e => e.owner_id === partnerId);
    ...
    for (const trade of partnerTrades) {
        ...
        name.innerText = trade.item.market_hash_name;
        left.appendChild(name);

        const detected = document.createElement('div');
        detected.style.paddingLeft = '45px';
        detected.style.color = 'darkgrey';
        detected.innerText = `Detected Trade (Float: ${trade.item.float_value.toFixed(12)}, Seed: ${trade.item.paint_seed})`;
        detected.style.color = 'darkgrey';
