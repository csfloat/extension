...
const isInventoryPage = function() {
    return /^\/(profile|id)\/\S*\/inventory/.test(window.location.pathname)
};

const extractInspectAssetId = function (link) {
    const m = decodeURIComponent(link).match(/^steam:\/\/rungame\/730\/\d+\/[+ ]csgo_econ_action_preview [SM]\d+A(\d+)D\d+$/);
    return m[0] && m[1];
};
