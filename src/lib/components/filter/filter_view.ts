...
export class FilterView extends FloatElement {
    ...
    protected render(): HTMLTemplateResult {
        return html`
            <div class="container">
                <input @input="${this.onColourChange}" class="colour-input" type="color" value="${this.filter.getColour()}">
                <div class="expression">${this.filter.getExpression()}</div>
                <csgofloat-steam-button .text="${"Global"}"
                                        .type="${this.filter.getIsGlobal() ? 'green_white' : 'grey_white'}"
                                        @click="${this.onToggleGlobal}"
                                        class="global-btn"></csgofloat-steam-button>
                <csgofloat-steam-button .text="${"Remove"}"
                                        .type="${'grey_white'}"
                                        @click="${this.onRemove}"
                                        class="remove-btn"></csgofloat-steam-button>
            </div>
        `;
    }
